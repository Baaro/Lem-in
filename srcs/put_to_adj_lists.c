...
static void		remember_start_end(t_adj_lists *adj_lists, t_rooms *room)
{
	if (room->start == TRUE)
	{
...
void		    put_to_adj_lists(t_map *map, t_adj_lists *adj_lists, char *data, size_t *cnt_rooms)
{
	...
	{
		if (!(adj_lists->rooms[id] = make_room(map, id, room[NAME], *cnt_rooms)))
			errors_memory(CANT_ALLOCATE_MEM);
		if (adj_lists->start == NULL || adj_lists->end == NULL)
			remember_start_end(adj_lists, adj_lists->rooms[id]);
